...
// VDA5050 spec §6.4: nodePosition.theta is given in degrees, range [-Pi..Pi]
// expressed as [-180..180].
...
// must match fleet_interface/src/vda5050_converter.cpp.
constexpr std::size_t kMaxVda5050Nodes = 10'000;

/**
 * Intermediate representation of a parsed VDA5050 nodePosition object.
 * Separating this from the ROS type keeps JSON parsing out of the pose-building
 * code.
 */
struct NodePosition {
  double x;
  double y;
  double theta_rad; // Heading in radians; 0.0 when absent from the JSON.
  bool has_theta;
};

/**
 * Extract a VDA5050 nodePosition from a single order-node JSON object.
 *
 * Returns false when the node lacks a "nodePosition" key or when that object
 * is missing the mandatory "x" or "y" fields. On false the output is undefined.
 */
...
/**
 * Construct a ROS PoseStamped from a parsed VDA5050 node position.
 *
 * Heading is encoded as a pure yaw quaternion: q = (0, 0, sin(θ/2), cos(θ/2)).
 * The z position is always 0 — VDA5050 operates in the 2-D floor plane.
 */
...
/**
 * Convert a list of Obstacle segments to a VDA5050 Information JSON object.
 * Each segment becomes an InfoReference with its geometry encoded as key/value.
 */
nlohmann::json obstacleSegmentsToInformation(
  ...
  for (std::size_t i = 0; i < obstacles.size(); ++i) {
    ...
    // Encode segment geometry as value string: "type;x1,y1;x2,y2;..."
    std::string geom;
...
std::string Vda5050Converter::obstacleReportToStateJson(
  ...
  // Determine eStop based on unmatched obstacles presence
